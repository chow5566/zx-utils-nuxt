...
<template>
  <div class="max-w-5xl mx-auto px-4 py-4 sm:py-8">
    ...
    <p class="text-sm sm:text-base text-(--ui-text-muted) mb-6 sm:mb-8">
      在线字数统计工具，实时统计文本的字符数、单词数、行数、段落数、字节数等指标。
    </p>
    ...
    <UCard class="mb-4 sm:mb-6">
      <template #header>
        <div class="flex items-center justify-between gap-3">
          <h2 class="text-base sm:text-lg font-semibold text-(--ui-text-highlighted)">文本输入</h2>
          <UButton
            v-show="input"
            @click="handleClear"
            variant="ghost"
    ...
    <UCard class="mb-4 sm:mb-6">
      <template #header>
        <h2 class="text-base sm:text-lg font-semibold text-(--ui-text-highlighted)">统计结果</h2>
      </template>
