...
<template>
  <div class="max-w-5xl mx-auto px-4 py-6 sm:py-10">
    ...
    <div class="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
      <div class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-md bg-cyan-50 dark:bg-cyan-950/40 shrink-0">
        <UIcon name="i-lucide-diff" class="w-5 h-5 sm:w-6 sm:h-6 text-cyan-600 dark:text-cyan-400" />
      </div>
      <div class="min-w-0">
        <h1 class="text-xl sm:text-2xl font-bold text-(--ui-text-highlighted)">文本差异对比</h1>
    ...
    <!-- Diff 结果面板 -->
    <div v-if="diffResult" class="rounded-md border border-(--ui-border) bg-(--ui-bg) overflow-hidden mb-5">
      <div class="flex items-center justify-between gap-3 px-5 py-3 border-b border-(--ui-border) bg-(--ui-bg-elevated)/50">
